...
public:
	...
	long CountryCount();
	bool CountrySelect(BSTR caCode[], BSTR caName[], int naSortOrder[]);

	bool VendorUpdate(int OpCode, char* Code, char* Name, char* Telephone, char* Fax, char* Contact, char* Email);
	long VendorCount();
	bool VendorSelect(BSTR bstrCode[], BSTR bstrName[],  BSTR bstrTelephone[], BSTR bstrFax[], BSTR bstrContact[], BSTR bstrEmail[]);

    bool PinUpdate(int OpCode, char* cpCode, char* cpPin, unsigned long ulBatchNumber);
 


	long PinCount();
	bool PinSelect(BSTR bstrCode[], BSTR bstrPin[], unsigned long ulBatchNumber[]);
